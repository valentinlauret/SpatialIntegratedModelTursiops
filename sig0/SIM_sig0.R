  ...
  # DS observation model
   ...
   for(s in 1:nsitesDS){
     ...
     for(g in 1:nD){ # midpt = mid-point of each bin
       ...
       p[s,g] <- p0.ds[s] * exp(- midpt[g] * midpt[g] / (2*sigma.ds*sigma.ds))
     ...
     logit(p0.ds[s])<- alpha0 + alpha1*seffDS[s]  + alpha2*sea.stateDS[s] # log-linear model detection
   }
...
save(samples,t, file ="SIM_sig0res2.rdata")
