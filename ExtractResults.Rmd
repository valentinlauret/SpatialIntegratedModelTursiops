...
load("~/Google Drive/GITHUB/Spatial-abundance-tursiops/SCR0trickGOOD40k.rdata")
denplot(samples2[,c("EN","mu0","mu1")])
# SCR
...
```{r echo=FALSE}
load("~/Google Drive/GITHUB/Spatial-abundance-tursiops/sea.grid.rdata")
...
```{r echo=FALSE}
load("~/Google Drive/GITHUB/Spatial-abundance-tursiops/pays.rdata")
library(cowplot)
library(ggplot2)
...
```{r include=FALSE}
load("~/Google Drive/GITHUB/Spatial-abundance-tursiops/SIM0trickres.rdata")
denplot(samples[,c("EN","mu1","mu0","alpha0","p0")])
SIPM_output <-  samples
...
for(i in 1:nrow(res3)){
    ...
    beta0sim[i] <- ch[i,"beta0"]
    beta1sim[i] <- ch[i,"beta1"]
    
    sigmascr_sim[i,] <-  exp(beta0sim[i] + beta1sim[i]*datSCR$seff[datSCR$seff>0])
